...
describe('Object copy', () => {
  ...
  it('rejects empty or invalid sourceKey', async () => {
    const daCtx = { users: [{ email: 'a@b.c' }] };
    const details = { source: 'src', destination: 'dst' };
    const resp1 = await copyFile({}, {}, daCtx, '', details, false);
    assert.strictEqual(resp1.$metadata.httpStatusCode, 400);
    const resp2 = await copyFile({}, {}, daCtx, 'bad//key', details, false);
    assert.strictEqual(resp2.$metadata.httpStatusCode, 400);
  });
