 ...
 */
export async function listCommand(daCtx, details) {
  ...
  const commandInput = { ...input, cursor: continuationToken };
  const r2list = await daCtx.env.DA_CONTENT.list(commandInput);

  sourceKeys.push(r2list.objects.map(({ key }) => key.split('/').slice(1).join('/')));

  return { sourceKeys, continuationToken: r2list.cursor };
}
